...
const Dex = new class implements ModdedDex {
	...
	resourcePrefix = (() => {
		...
		return `${prefix}//${window.Config ? Config.routes.client : 'protected-sands-88246.herokuapp.com'}/`;
	})();

	fxPrefix = (() => {
		if (window.document?.location?.protocol === 'file:') {
			if (window.Replays) return `https://${window.Config ? Config.routes.client : 'protected-sands-88246.herokuapp.com'}/fx/`;
			return `fx/`;
		}
		return `//${window.Config ? Config.routes.client : 'protected-sands-88246.herokuapp.com'}/fx/`;
	})();
